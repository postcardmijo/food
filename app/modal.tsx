...
import { Alert, Platform, Pressable, ScrollView, StyleSheet, useColorScheme, View } from 'react-native';
...
const FOOD_ITEMS: Record<string, any[]> = {
  "The Grill": [
    { id: 'g1', name: "Cheeseburger", protein: 25, carbs: 30, fat: 18, calories: 450 },
    { id: 'g2', name: "Grilled Chicken Breast", protein: 30, carbs: 0, fat: 4, calories: 180 },
    { id: 'g3', name: "Fries (Small)", protein: 3, carbs: 40, fat: 14, calories: 320 },
  ],
  "Pasta Station": [
    { id: 'p1', name: "Alfredo Pasta", protein: 12, carbs: 60, fat: 22, calories: 550 },
    { id: 'p2', name: "Marinara Pasta", protein: 8, carbs: 55, fat: 8, calories: 380 },
    { id: 'p3', name: "Garlic Bread", protein: 4, carbs: 20, fat: 8, calories: 180 },
  ],
  "default": [
    { id: 'd1', name: "Generic Meal A", protein: 20, carbs: 40, fat: 10, calories: 400 },
    { id: 'd2', name: "Generic Meal B", protein: 15, carbs: 30, fat: 12, calories: 350 },
  ],
};
...
export default function ModalScreen() {
  ...
  const handleAddFood = () => {
    ...
    console.log("Adding Food:", { hall: selectedHall, station: selectedStation, totals });
    router.dismiss();
  };
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.pillScroll}>
          {DINING_HALLS.map((hall) => (
            <Pressable
              key={hall}
              onPress={() => { setSelectedHall(hall); setSelectedStation(null); setSelectedFoodIds(new Set()); }}
              style={[
                styles.pill, 
                { backgroundColor: selectedHall === hall ? theme.primary : theme.pillBg }
              ]}
            >
              <ThemedText 
                style={[
                  styles.pillText, 
                  { color: selectedHall === hall ? '#fff' : theme.pillText, fontWeight: selectedHall === hall ? 'bold' : 'normal' }
                ]}
              >
                {hall}
              </ThemedText>
          ...
          <>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.pillScroll}>
              {STATIONS[selectedHall].map((station) => (
                <Pressable
                  key={station}
                  onPress={() => { setSelectedStation(station); setSelectedFoodIds(new Set()); }}
                  style={[
                    styles.pill, 
                ...
                >
                  <ThemedText 
                    style={[
                      styles.pillText, 
                      { color: selectedStation === station ? '#fff' : theme.pillText, fontWeight: selectedStation === station ? 'bold' : 'normal' }
                    ]}
                  >
                    {station}
                  </ThemedText>
        ...
        {selectedStation && (
          <>
            <View style={styles.listHeader}>
                ...
                <ThemedText style={{ fontSize: 12, color: theme.textSecondary }}>(Tap to select)</ThemedText>
            </View>
